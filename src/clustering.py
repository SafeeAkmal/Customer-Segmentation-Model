...
def run_kmeans(X: np.ndarray, n_clusters: int = 5, random_state: int = 42) -> Tuple[KMeans, np.ndarray, Dict[str, float]]:
...
def run_hierarchical(X: np.ndarray, n_clusters: int = 5, linkage: str = "ward") -> Tuple[AgglomerativeClustering, np.ndarray, Dict[str, float]]:
...
def dbscan_grid_search(X: np.ndarray,
                       eps_values: list = [0.3, 0.5, 0.8, 1.0, 1.5],
                       min_samples_values: list = [3, 5, 7, 10]) -> pd.DataFrame:
    results = []
